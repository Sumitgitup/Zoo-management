...
export const TheWildLifeV2 = ({
  ...
  // Refs for scroll effects and intersection observer
  ...
  const isCardsInView = useInView(cardsRef, { amount: 0.3, once: true });
  ...
  const getCardProps = useCallback(
    (index: number) => {
      ...
      if (isActive) {
        return { x: 0, y: 0, scale: 1, rotateY: 0, zIndex: 30, opacity: 1 };
      } else if (isNext) {
        return {
          x: 50,
          y: 20,
        ...
        return { x: 0, y: 40, scale: 0.8, rotateY: 0, zIndex: 1, opacity: 0.3 };
      }
    },
  ...
  return (
    ...
    >
      <div className="relative grid grid-cols-1 gap-10 md:grid-cols-2 lg:gap-52">
        <motion.div
          ref={textRef}
          className="order-2 md:order-1 flex flex-col justify-center items-center md:items-start md:mr-5 text-center md:text-left py-4"
        ...
        >
          ...
          <div className="min-h-[320px] flex flex-col justify-between w-full max-w-lg">
            ...
            <div className="flex items-center gap-4 mt-8 justify-center md:justify-start">
              ...
              >
                <IconArrowRight className="h-4 w-4 text-white dark:text-gray-900 transition-transform duration-200 group-hover:translate-x-0.5" />
              </button>
            </div>
          </div>
        </motion.div>
